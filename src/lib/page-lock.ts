...
import { formatDeadlineDisplay } from "@/lib/timezone";
import type { GameConfig, PlayPage } from "@/types";

/** 各页默认自动锁定时间（UTC ISO）。 */

export const DEFAULT_PAGE_LOCKS_AT: Record<PlayPage, string> = {
    1: "2026-06-29T17:00:00.000Z",
    2: "2026-07-04T17:00:00.000Z",
    3: "2026-07-09T20:00:00.000Z"
};

/** 历史默认截止时间；线上 config 若仍为这些值且未被管理员手动解锁，会自动迁移到 {@link DEFAULT_PAGE_LOCKS_AT}。 */

const STALE_PAGE_LOCKS_AT: Record<PlayPage, readonly string[]> = {
    1: ["2026-06-18T15:00:00.000Z", "2026-06-28T17:30:00.000Z", "2026-06-28T19:00:00.000Z"],
    2: ["2026-07-04T15:30:00.000Z", "2026-06-19T13:00:00.000Z"],
    3: ["2026-07-09T18:30:00.000Z"]
};
export function migratePageLockSchedule(config: GameConfig): {
    config: GameConfig;
    changed: boolean;
} {
    let changed = false;
    ...
    (([1, 2, 3] as const) satisfies PlayPage[]).forEach((page) => {
        ...
        const current = config[fields.locksAt];
        const target = DEFAULT_PAGE_LOCKS_AT[page];
        if (!current || current === target)
            return;
        if (!STALE_PAGE_LOCKS_AT[page].includes(current))
            return;
        if (!changed)
            next = { ...config };
        next = { ...next, [fields.locksAt]: target };
        changed = true;
    });
...
export function isPageAutoLocked(config: GameConfig, page: PlayPage, now = Date.now()): boolean {
    if (config[lockFields(page).manualLocked])
        return false;
    return isScheduleLocked(config, page, now);
}
export function applyManualPageLock(config: GameConfig, page: PlayPage, locked: boolean): GameConfig {
    const fields = lockFields(page);
    if (locked) {
        return {
            ...config,
...
export function defaultPageLockSchedule(): Pick<GameConfig, "page1LocksAt" | "page2LocksAt" | "page3LocksAt" | "page1LockOverridden" | "page2LockOverridden" | "page3LockOverridden"> {
    return {
...
export function formatPageDeadlineDisplay(iso: string | null, locale: Locale, timeZone = "UTC"): string | null {
    return formatDeadlineDisplay(iso, locale, timeZone);
}
