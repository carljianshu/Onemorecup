...
export const EARLY_ANSWERS_MARKET_ID = "m1-1";
export function defaultAnswersPageSchedule(): Pick<GameConfig, "answersPage1Public" | "answersPage2Public" | "answersPage3Public" | "answersPage1OpensAt" | "answersPage2OpensAt" | "answersPage3OpensAt" | "answersPage1ScheduleOpenApplied" | "answersPage2ScheduleOpenApplied" | "answersPage3ScheduleOpenApplied" | "answersM1_1Public"> {
    return {
        ...
        answersM1_1Public: false
    };
}
function answersScheduleAppliedField(page: PlayPage) {
    if (page === 1)
        return "answersPage1ScheduleOpenApplied" as const;
    if (page === 2)
        return "answersPage2ScheduleOpenApplied" as const;
    return "answersPage3ScheduleOpenApplied" as const;
}
function answersOpensAtField(page: PlayPage) {
    if (page === 1)
        return "answersPage1OpensAt" as const;
    if (page === 2)
        return "answersPage2OpensAt" as const;
    return "answersPage3OpensAt" as const;
}
function answersPublicField(page: PlayPage) {
    if (page === 1)
        return "answersPage1Public" as const;
    if (page === 2)
        return "answersPage2Public" as const;
...
export function migrateAnswersScheduleOpenApplied(config: GameConfig, now = Date.now()): {
    config: GameConfig;
    changed: boolean;
} {
    let changed = false;
    ...
    (([1, 2, 3] as const) satisfies PlayPage[]).forEach((page) => {
        const appliedKey = answersScheduleAppliedField(page);
        if (typeof config[appliedKey] === "boolean")
            return;
...
export function applyScheduledAnswersPageOpen(config: GameConfig, now = Date.now()): {
    config: GameConfig;
    changed: boolean;
} {
    let changed = false;
    ...
    (([1, 2, 3] as const) satisfies PlayPage[]).forEach((page) => {
        ...
        if (!opensAt)
            return;
        if (now < new Date(opensAt).getTime())
            return;
        if (config[appliedKey])
            return;
        if (!changed)
            next = { ...config };
        next = {
...
export function patchAnswersPageSchedule(config: GameConfig, feature: AnswersPageFeature, patch: {
    public?: boolean;
    opensAt?: string | null;
}): GameConfig {
...
export function isMarketAnswersPublic(config: GameConfig, marketId: string, page: PlayPage): boolean {
    if (isEarlyMarketAnswersPublic(config, marketId))
        return true;
    return isAnswersPagePublic(config, page);
}
export function listPublicAnswerMarketIds(markets: {
    id: string;
    page: PlayPage;
}[], config: GameConfig) {
    return markets
        .filter((market) => isMarketAnswersPublic(config, market.id, market.page))
        .map((market) => market.id);
}
export function isAnswersAnyPublic(config: GameConfig) {
    return (config.answersM1_1Public ||
